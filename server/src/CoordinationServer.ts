...
export class CoordinationServer {
  ...
  private buildDeltaContext(
  ...
  ): { startSeq: number; contextMessages: Message[]; renameNotices: string[] } {
    const seen = this.seenState.get(agent.participantId);
    const lastSeen = seen?.lastAckedDeliveredSeq ?? 0;
    const isFirstDelivery = lastSeen === 0;

    const renameNotices = this.getRenameNotices(agent);

    if (isFirstDelivery) {
      const recentMessages = this.transcript
    ...
    const unseenMessages = this.transcript.filter(
  ...
  private getRenameNotices(agent: Participant): string[] {
    const seen = this.seenState.get(agent.participantId);
    if (!seen) return [];

    return this.renameEvents
  ...
  private formatAgentPrompt(
    ...
    const owner = this.participants.find(p => p.participantId === agent.ownerHumanId);
    const author = this.participants.find(p => p.participantId === currentMessage.authorParticipantId);

    const participantList = this.participants.map(p => {
      const ownerName = p.ownerHumanId
        ? this.participants.find(o => o.participantId === p.ownerHumanId)?.displayName || 'unknown'
        : '';
    ...
    const contextSection = deltaContext.contextMessages.length > 0
      ? deltaContext.contextMessages.map(m => {
          const mAuthor = this.participants.find(p => p.participantId === m.authorParticipantId);
          const mRecipient = m.recipientParticipantId
            ? this.participants.find(p => p.participantId === m.recipientParticipantId)
            : null;
          const recipientAttr = mRecipient ? ` to="${this.escapeXml(mRecipient.displayName)}"` : '';
          return `<message seq="${m.seq}" from="${this.escapeXml(mAuthor?.displayName || 'unknown')}" kind="${mAuthor?.kind || 'unknown'}"${recipientAttr}>\n${this.escapeXml(m.parsedBody)}\n</message>`;
        }).join('\n\n')
      : '';

    const isFirstDelivery = (this.seenState.get(agent.participantId)?.lastAckedDeliveredSeq ?? 0) === 0;
    ...
    const planOnlySection = this.session?.agentMode === 'plan-only'
...
<current_message
  ...
  from="${this.escapeXml(author?.displayName || 'unknown')}"
  to="${this.escapeXml(agent.displayName)}"
  seq="${currentMessage.seq}">
${this.escapeXml(currentMessage.parsedBody)}
</current_message>
