...
import { CommandTraceWriter } from '../../src/local-boost-runtime/CommandTraceWriter';
import { LocalBoostTrace, CLAUI_LOCAL_BOOST_SCHEMA_VERSION } from '../../src/extension/local-boost/LocalBoostTypes';
...
// ---------------------------------------------------------------------------
// Helper: build a minimal valid LocalBoostTrace object
// ---------------------------------------------------------------------------
function buildTrace(overrides: Partial<LocalBoostTrace> = {}): LocalBoostTrace {
  return {
...
describe('CommandTraceWriter', () => {
  ...
  describe('writeTrace', () => {
    it('creates a valid JSON file in traces/YYYY-MM-DD/', async () => {
      ...
      const parsed = JSON.parse(content) as LocalBoostTrace;
      ...
      assert.equal(parsed.schemaVersion, CLAUI_LOCAL_BOOST_SCHEMA_VERSION);
    });
