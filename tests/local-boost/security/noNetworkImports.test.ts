...
// local-boost-runtime directory. This runtime executes shell commands on
// the user's machine and must NEVER make network calls.
// ---------------------------------------------------------------------------

const RUNTIME_DIR = path.resolve(__dirname, '../../../src/local-boost-runtime');
...
describe('No-network static analysis for local-boost-runtime', () => {
  ...
  it('should not import any banned networking modules', () => {
    ...
    if (violations.length > 0) {
      ...
      assert.fail(
        `Found ${violations.length} banned network import(s) in local-boost-runtime:\n${report}\n\n` +
        `The local-boost-runtime must be fully offline. Remove these imports.`,
      );
    }
  });
});
