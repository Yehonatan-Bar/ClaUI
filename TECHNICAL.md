...
**Tab Layout (Horizontal / Vertical)** - Setting `claudeMirror.tabs.layout` controls how ClaUi session tabs are navigated. Horizontal (default) keeps VS Code's native editor tab strip. Vertical collapses editor splits into one full-height group, hides VS Code's native tabs (`workbench.editor.showTabs` -> `'none'`), and renders a resizable in-webview vertical tab rail (drag right edge to resize, double-click to reset). The rail is rendered at the `App` level so it appears on all tab kinds (chat, search, multiparticipant). Three entry points: bottom-toolbar **View -> Tab layout** segmented control (`StatusBar.tsx`), Sessions title-bar gear (`claudeMirror.tabs.openLayoutMenu` QuickPick), and the Settings UI. `TabManager` registers an `onDidChangeConfiguration` listener that auto-calls `applyTabLayout()` so all three entry points apply the same behavior.
