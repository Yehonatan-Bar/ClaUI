...
1. **Entry point** — `StatusBar -> Tools` dropdown adds a "Smart Search"
   ...
   Codex"). The Claude rows are fixed (Opus 4.7, Sonnet 4.6, Haiku 4.5). The
   Codex rows are built from `codexModelOptions`, which both message handlers
   read from `~/.codex/models_cache.json`; only when that cache is unreadable
   does StatusBar fall back to a static list. This keeps the menu from
   offering models the current account cannot actually run. Click dispatches
   `{ type: 'openSmartSearch', provider, model }` to the active tab's
   MessageHandler.
