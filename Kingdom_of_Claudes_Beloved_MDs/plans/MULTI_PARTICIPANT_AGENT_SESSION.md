...
For Codex, if a delivery arrives while a turn is running, the bridge may call the existing Codex `steer` path directly (equivalent to sending with `steer: true`). It should not first send a normal prompt and then simulate a second button press. The delivery is a single server delivery with a busy policy of `codex-auto-steer`.
...
1. The AgentBridge sends the prompt once with `steer: true`.
2. The current Codex turn is interrupted using the existing steer behavior.
3. The interrupted delivery is marked `interrupted`, with `interruptedByDeliveryId = newDeliveryId`.
4. The new delivery proceeds as the active Codex turn.
5. The prompt template includes a short notice that the agent was steered/interrupted by a newer current task.

This is the same user-facing behavior as pressing the Steer button manually, but automated by the bridge after the original human or server delivery request. It is not implemented as "send normally, then press Steer"; it is one delivery using the steer path.
...
Auto-steer needs a small guard against rapid repeated interruptions:

- If a Codex auto-steer is already in progress, do not start a second cancellation immediately.
- Coalesce additional pending deliveries for a short debounce window (for example 500-1000ms), or ask the user whether to replace the pending steer target.
- Never allow an agent-to-agent loop to repeatedly auto-steer the same Codex agent without passing through the A2A loop-control rules.
- Show all humans that delivery X interrupted delivery Y.
