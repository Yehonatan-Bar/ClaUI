# ClaUi Local Boost - Full Implementation Plan

**Spec source:** `C:\projects\CLAUI_LOCAL_BOOST_TECHNICAL_SPEC.md`
...
```
src/extension/local-boost/
src/local-boost-runtime/
src/local-boost-runtime/filters/
src/local-boost-runtime/hooks/
```

### 1.2 Create `src/extension/local-boost/LocalBoostTypes.ts`

All shared types used across extension and runtime:

```ts
// LocalBoostTrace - full trace schema (spec section 11.3)
// LocalBoostTraceSummary - lightweight version for UI
// LocalBoostStatus - current feature state
// LocalBoostAggregate - aggregated stats
// LocalBoostSessionStats - per-session stats (spec section 17.7)
// LocalBoostContextFile - context JSON schema (spec section 10.4)
...
// LocalBoostEnvInput - env builder input (spec section 10.1)

export const CLAUI_LOCAL_BOOST_VERSION = '1.0.0';
export const CLAUI_LOCAL_BOOST_SCHEMA_VERSION = 1;
```

### 1.3 Create `src/extension/local-boost/LocalBoostSettings.ts`

Read settings from `vscode.workspace.getConfiguration('claudeMirror.localBoost')`.

```ts
export interface LocalBoostSettings {
  enabled: boolean;
...
export function getLocalBoostSettings(): LocalBoostSettings;
export function onSettingsChanged(callback: (settings: LocalBoostSettings) => void): vscode.Disposable;
```
...
```json
"claudeMirror.localBoost.enabled": { "type": "boolean", "default": false, ... },
"claudeMirror.localBoost.filterProfile": { "type": "string", "enum": ["balanced","strict","verbose"], "default": "balanced" },
"claudeMirror.localBoost.storeRawRedactedLogs": { "type": "boolean", "default": true },
"claudeMirror.localBoost.rawLogRetentionDays": { "type": "number", "default": 7, "min": 1, "max": 90 },
"claudeMirror.localBoost.maxRawLogMb": { "type": "number", "default": 100, "min": 10, "max": 5000 },
"claudeMirror.localBoost.traceRetentionDays": { "type": "number", "default": 30, "min": 1, "max": 365 },
"claudeMirror.localBoost.maxTraceCount": { "type": "number", "default": 10000, "min": 100, "max": 100000 },
"claudeMirror.localBoost.dailyReportRetentionDays": { "type": "number", "default": 90, "min": 7, "max": 365 },
"claudeMirror.localBoost.workspaceLocalStorage": { "type": "boolean", "default": false, "description": "Store traces/logs in <workspace>/.claui/local-boost/ instead of global storage. Opt-in because it writes files inside the project." },
"claudeMirror.localBoost.installClaudeHook": { "type": "boolean", "default": false },
"claudeMirror.localBoost.installCodexHook": { "type": "boolean", "default": false },
"claudeMirror.localBoost.codexMode": { "type": "string", "enum": ["off","instruction-only","hook-guard"], "default": "instruction-only" }
```
...
### 2.1 Create `src/local-boost-runtime/SecretRedactor.ts`
...
5. **Fail-closed (spec 12.1, 12.6):** If redaction throws, return `{ text: '[claui-local-boost] Output suppressed: redaction error.', replacements: 0, rulesTriggered: ['ERROR'] }`.

### 2.2 Tests — `tests/local-boost/SecretRedactor.test.ts`
...
### 3.1 Create filter interfaces — `src/local-boost-runtime/filters/OutputFilterRegistry.ts`
...
9. Prepend output header (spec 13.3):
   ```
   [claui-local-boost] <command> <passed/failed> with exit code <N> in <duration>.
...
### 3.6 Tests — `tests/local-boost/filters/`
...
### 4.1 Create `src/local-boost-runtime/CommandEligibility.ts`

Also create a copy/shared version at `src/extension/local-boost/CommandEligibility.ts` if the hook scripts need a standalone module (the hook scripts run outside the extension process).
...
3. Check for `CLAUI_LOCAL_BOOST_BYPASS=1` in the command string — if present, not eligible
...
### 4.2 Tests — `tests/local-boost/CommandEligibility.test.ts`
...
### 5.1 Create `src/local-boost-runtime/CommandTraceWriter.ts`

```ts
export class CommandTraceWriter {
  constructor(private storeDir: string);

  writeTrace(trace: LocalBoostTrace): Promise<void>;
...
### 5.2 Create `src/extension/local-boost/LocalBoostContextStore.ts`

Manages context files that bridge the extension and the runner (spec 10.4).

```ts
export class LocalBoostContextStore {
  constructor(private storeDir: string);
...
### 5.3 Create `src/extension/local-boost/LocalBoostTraceReader.ts`

Reads traces for the UI dashboard.

```ts
export class LocalBoostTraceReader {
  constructor(private storeDir: string);

  getRecentTraces(limit: number, workspacePath?: string): Promise<LocalBoostTraceSummary[]>;
  getTrace(traceId: string): Promise<LocalBoostTrace | null>;
  getAggregate(workspacePath?: string): Promise<LocalBoostAggregate>;
  getRawLog(traceId: string, stream: 'stdout' | 'stderr'): Promise<string | null>;
  getDailyReport(date: string): Promise<LocalBoostDailyReport | null>;
  cleanExpired(settings: LocalBoostSettings): Promise<{ deletedTraces: number; deletedRawLogs: number; deletedReports: number; freedBytes: number }>;
}
```

### 5.4 Create `src/extension/local-boost/LocalBoostDailyReportGenerator.ts`
...
```ts
export class LocalBoostDailyReportGenerator {
  constructor(private storeDir: string, private traceReader: LocalBoostTraceReader);

  async generateDailyReport(date: string): Promise<LocalBoostDailyReport>;
  async generateIfMissing(date: string): Promise<void>;
}

type LocalBoostDailyReport = {
  schemaVersion: 1;
  date: string;
  generatedAt: string;
  commandCount: number;
...
- On `LocalBoostService.dispose()`, generate report for current day (partial)
...
The `cleanExpired()` method in `LocalBoostTraceReader` must enforce all three retention policies:
...
When `workspaceLocalStorage` is enabled, storage root moves to `<workspace>/.claui/local-boost/` instead of `<globalStorageUri>/local-boost/`.
...
- `tests/local-boost/CommandTraceWriter.test.ts`: atomic write, directory creation, concurrent writes
- `tests/local-boost/LocalBoostContextStore.test.ts`: create/update/dispose, corrupt file handling
- `tests/local-boost/LocalBoostTraceReader.test.ts`: read back written traces, aggregation, retention cleanup for all three tiers
- `tests/local-boost/LocalBoostDailyReportGenerator.test.ts`: report generation from traces, idempotent generation, partial day reports
...
### 6.1 Create `src/local-boost-runtime/NoNetworkGuard.ts`
...
### 6.2 Create `src/local-boost-runtime/executeShellCommand.ts`
...
### 6.3 Create `src/local-boost-runtime/cli.ts`

This is the entry point for `claui-run`. It will compile to `dist/local-boost-runtime/cli.js`.
...
4. Read env vars:
   CLAUI_LOCAL_BOOST_CONTEXT_FILE -> read context JSON
   CLAUI_LOCAL_BOOST_STORE_DIR -> trace/log storage root
   CLAUI_LOCAL_BOOST_SHELL -> shell override (optional)
...
- If decode fails: print `[claui-local-boost] Runner failed before executing command: <reason>`, exit 127
...
```sh
#!/usr/bin/env sh
exec node "<absolute-path>/dist/local-boost-runtime/cli.js" "$@"
```

**`claui-run.cmd` (Windows):**
```bat
@echo off
node "<absolute-path>\dist\local-boost-runtime\cli.js" %*
```

### 6.5 Tests — `tests/local-boost/runner/`
...
- Runner works from terminal: `node dist/local-boost-runtime/cli.js -- echo hello`
...
**Goal:** Wire Local Boost into the extension lifecycle. The service layer manages installation, environment building, and state.

### 7.1 Create `src/extension/local-boost/LocalBoostInstaller.ts`
...
interface LocalBoostRuntimePaths {
...
1. Check `<globalStorage>/local-boost/runtime/version.json` for current version
2. If missing or version mismatch, copy compiled runtime files from `<extensionPath>/dist/local-boost-runtime/` to global storage
...
### 7.2 Create `src/extension/local-boost/LocalBoostEnvBuilder.ts`
...
**Implementation:**
1. Clone `baseEnv`
2. Set all `CLAUI_LOCAL_BOOST_*` env vars (spec 10.2)
3. Prepend `binDir` to PATH
4. Save original PATH as `CLAUI_LOCAL_BOOST_ORIGINAL_PATH`
...
### 7.3 Create `src/extension/local-boost/LocalBoostService.ts`
...
export class LocalBoostService implements vscode.Disposable {
  ...
  getContextStore(): LocalBoostContextStore;
  getTraceReader(): LocalBoostTraceReader;
  getHookManager(): LocalBoostHookManager;
...
```ts
const localBoostService = new LocalBoostService(context);
await localBoostService.initialize();
context.subscriptions.push(localBoostService);
```

Pass `localBoostService` to process managers and message handlers that need it.

### 7.5 Tests

- `tests/local-boost/LocalBoostEnvBuilder.test.ts`: env vars set correctly, PATH prepended, telemetry vars removed, original PATH preserved
- `tests/local-boost/LocalBoostInstaller.test.ts`: runtime created, version check works, update replaces files
...
**Goal:** Modify existing process managers to inject Local Boost environment when enabled.

### 8.1 Modify `src/extension/process/ClaudeProcessManager.ts`

**Minimal changes:**

1. Accept `LocalBoostService` (or null) in constructor or via setter
2. Before spawning Claude CLI (`child_process.spawn`), check if Local Boost is enabled
3. If enabled:
   ```ts
   const env = this.localBoostService.buildAgentEnv({
     baseEnv: process.env,
     provider: 'claude',
...
5. If instruction fallback is needed (no hook installed), append Local Boost instruction to system prompt if that mechanism exists
...
1. If `codexMode === 'instruction-only'`, append Local Boost usage instruction to Codex `-c instructions=` parameter
...
4. If `codexMode === 'off'`, skip all Local Boost integration
...
Add aggregate Local Boost stats to session summaries (spec 17.7). This connects per-session command compression data to the existing analytics pipeline.

1. Add optional field to session summary type:
   ```ts
   localBoost?: LocalBoostSessionStats;
   ```

2. Define `LocalBoostSessionStats` (in LocalBoostTypes.ts, referenced here):
   ```ts
   type LocalBoostSessionStats = {
     commandCount: number;
     failedCommandCount: number;
     totalRawBytes: number;
     totalFilteredBytes: number;
...
3. When a session ends, aggregate trace data for that session's `tabRuntimeId` via `LocalBoostTraceReader.getAggregate()` and attach to the session summary.

4. **Backward compatibility:** Old summaries without `localBoost` field must load without errors. Use optional chaining throughout.

5. **Do NOT feed raw/filtered command output into ActivitySummarizer.** Only allowed integration: `recordToolUse("Bash (npm test, compressed by ClaUi Local Boost)")` — no output content.

### 8.6 Modify `src/webview/hooks/useClaudeStream.ts`

If this hook processes incoming stream events to update the store, add handling for Local Boost aggregate updates. When the extension sends `localBoostAggregateUpdate` or `localBoostTraceUpdate` messages, forward them to the store's Local Boost slice.

### 8.7 Acceptance

- Claude sessions start with Local Boost env vars when enabled
- Codex sessions start with Local Boost env vars when enabled
- Context files are created and updated during session lifecycle
- Session summaries in ProjectAnalyticsStore include `localBoost` stats when the feature is active
- Old session summaries without `localBoost` field load without errors
- Existing Claude/Codex flows work identically when Local Boost is disabled (all changes are gated)
...
**`src/local-boost-runtime/hooks/claudePreToolUse.ts`**
...
5. If command starts with "claui-run" or contains CLAUI_LOCAL_BOOST_BYPASS=1:
...
**`src/local-boost-runtime/hooks/codexPreToolUse.ts`**
...
### 9.2 Create `src/extension/local-boost/LocalBoostHookManager.ts`

```ts
export class LocalBoostHookManager {
  constructor(
    private runtimePaths: LocalBoostRuntimePaths,
...
- `tests/local-boost/hooks/claudePreToolUse.test.ts`: mock stdin/stdout, verify rewrite for eligible commands, passthrough for ineligible
- `tests/local-boost/hooks/codexPreToolUse.test.ts`: mock stdin/stdout, verify deny output for eligible commands
- `tests/local-boost/LocalBoostHookManager.test.ts`: install/uninstall, merge behavior, backup creation, idempotent install, clean uninstall
...
**Goal:** Add Local Boost status, settings, and trace viewer to the ClaUi webview.
...
```ts
// Webview -> Extension
| { type: 'localBoostGetStatus' }
| { type: 'localBoostSetEnabled'; enabled: boolean }
| { type: 'localBoostInstallHooks'; provider: 'claude' | 'codex' | 'both' }
| { type: 'localBoostUninstallHooks'; provider: 'claude' | 'codex' | 'both' }
| { type: 'localBoostOpenTrace'; traceId: string }
| { type: 'localBoostClearData'; scope: 'workspace' | 'all' }
| { type: 'localBoostExportReport'; scope: 'workspace' | 'all' }

// Extension -> Webview
| { type: 'localBoostStatus'; status: LocalBoostStatus }
| { type: 'localBoostTraceUpdate'; trace: LocalBoostTraceSummary }
| { type: 'localBoostAggregateUpdate'; aggregate: LocalBoostAggregate }
| { type: 'localBoostError'; error: string }
```

### 10.2 Update message handlers

In `src/extension/webview/MessageHandler.ts` and `src/extension/webview/CodexMessageHandler.ts`:
- Add case handlers for `localBoost*` messages
- Route to `LocalBoostService` methods
- Return status/trace/aggregate data to webview

### 10.3 Update webview store — `src/webview/state/store.ts`

Add Local Boost state slice:

```ts
localBoost: {
  status: LocalBoostStatus | null;
  recentTraces: LocalBoostTraceSummary[];
  aggregate: LocalBoostAggregate | null;
  error: string | null;
};
```

Add actions: `setLocalBoostStatus`, `addLocalBoostTrace`, `setLocalBoostAggregate`, `setLocalBoostError`.

### 10.4 Create UI components

**`src/webview/components/LocalBoost/`** (new directory)

| Component | Purpose |
|---|---|
| `LocalBoostStatusBadge.tsx` | Small badge in status bar area: "Local Boost: Off/On - N cmds - ~Xk tokens saved" |
| `LocalBoostSettingsPanel.tsx` | Toggle enable, hook install/uninstall buttons, codex mode, filter profile, clear data |
| `LocalBoostTracePanel.tsx` | Table of recent traces: time, provider, command family, exit, duration, raw/filtered bytes, tokens saved, filter, redactions |
| `LocalBoostTraceDetail.tsx` | Expanded view of single trace with redacted log viewer |

### 10.5 Integrate into existing layout

- Add `LocalBoostStatusBadge` to the `StatusBar` component (or `VitalsInfoPanel` area)
- Add `LocalBoostSettingsPanel` as a collapsible section in the Vitals dashboard or a dedicated tab
- Add `LocalBoostTracePanel` as a section in Vitals/Analytics or as its own dashboard tab
...
|---|---|---|
| Node runtime not found for claui-run | "Local Boost: Node.js required" | Link to Node.js install instructions |
| Claude hook not installed | "Local Boost: Hook not installed for this workspace" | "Install Hook" button |
| Codex hooks not enabled/trusted | "Local Boost: Advisory mode (Codex hooks unavailable)" | Show codexMode setting |
| Runner failed | "Local Boost: Runner error — [reason]" | "View Details" expands to show diagnostic |
| Redaction failed closed | "Local Boost: Output suppressed (redaction error)" | "View Trace" link to the affected trace |
| Trace storage unavailable | "Local Boost: Cannot write traces — [path]" | "Open Folder" to show the storage location |
| Existing hook settings invalid JSON | "Local Boost: Cannot install hook — [path] is invalid JSON" | "Open File" to let user fix it |
...
- Trace panel shows recent commands after running an agent session with Local Boost enabled
...
Ensure `dist/local-boost-runtime/` is included in the VSIX package.
...
- Installed extension folder at `~/.vscode/extensions/` contains `dist/local-boost-runtime/`
...
Create `tests/local-boost/security/noNetworkImports.test.ts`:

Scan all files in `src/local-boost-runtime/` for imports of banned modules:
```
...
Create `tests/local-boost/security/noNetworkRuntime.test.ts`:
...
Create `tests/local-boost/security/redactionIntegration.test.ts`:
...
|---|---|---|
| SecretRedactor | `tests/local-boost/SecretRedactor.test.ts` | 10+ cases (Phase 2) |
| GenericFilter | `tests/local-boost/filters/GenericFilter.test.ts` | ANSI strip, dedup, budgets |
| JS Package Filter | `tests/local-boost/filters/JavaScriptPackageFilter.test.ts` | npm test output compression |
| Pytest Filter | `tests/local-boost/filters/PytestFilter.test.ts` | pytest output compression |
| Jest/Vitest Filter | `tests/local-boost/filters/JestVitestFilter.test.ts` | jest output compression |
| TypeScript Filter | `tests/local-boost/filters/TypeScriptFilter.test.ts` | tsc diagnostic grouping |
| ESLint Filter | `tests/local-boost/filters/EslintFilter.test.ts` | eslint diagnostic grouping |
| Playwright Filter | `tests/local-boost/filters/PlaywrightFilter.test.ts` | playwright output compression (post-MVP) |
| Docker Filter | `tests/local-boost/filters/DockerFilter.test.ts` | docker build/logs compression (post-MVP) |
| Go Filter | `tests/local-boost/filters/GoFilter.test.ts` | go test output compression (post-MVP) |
| Maven/Gradle Filter | `tests/local-boost/filters/MavenGradleFilter.test.ts` | mvn/gradle compression (post-MVP) |
| Cargo Filter | `tests/local-boost/filters/CargoFilter.test.ts` | cargo test/build compression (post-MVP) |
| GitDiff Filter | `tests/local-boost/filters/GitDiffFilter.test.ts` | large diff compression (post-MVP) |
| DailyReportGenerator | `tests/local-boost/LocalBoostDailyReportGenerator.test.ts` | report generation, idempotency |
| CommandEligibility | `tests/local-boost/CommandEligibility.test.ts` | 15+ cases (Phase 4) |
| CommandTraceWriter | `tests/local-boost/CommandTraceWriter.test.ts` | atomic writes |
| LocalBoostContextStore | `tests/local-boost/LocalBoostContextStore.test.ts` | lifecycle |
| LocalBoostEnvBuilder | `tests/local-boost/LocalBoostEnvBuilder.test.ts` | env vars |
| Hook: Claude | `tests/local-boost/hooks/claudePreToolUse.test.ts` | rewrite/passthrough |
| Hook: Codex | `tests/local-boost/hooks/codexPreToolUse.test.ts` | deny/passthrough |
| HookManager | `tests/local-boost/LocalBoostHookManager.test.ts` | install/uninstall |
| No-network static | `tests/local-boost/security/noNetworkImports.test.ts` | import scan |
| No-network runtime | `tests/local-boost/security/noNetworkRuntime.test.ts` | monkeypatch |
| Redaction integration | `tests/local-boost/security/redactionIntegration.test.ts` | end-to-end |
...
| Resumed session | Claude | Local Boost still active, context updated |
...
- [ ] No network imports in Local Boost runtime
...
src/extension/local-boost/
  LocalBoostTypes.ts
  LocalBoostSettings.ts
  LocalBoostService.ts
  LocalBoostInstaller.ts
  LocalBoostEnvBuilder.ts
  LocalBoostContextStore.ts
  LocalBoostHookManager.ts
  LocalBoostTraceReader.ts
  LocalBoostDailyReportGenerator.ts
...
src/webview/components/LocalBoost/
  LocalBoostStatusBadge.tsx
  LocalBoostSettingsPanel.tsx
  LocalBoostTracePanel.tsx
  LocalBoostTraceDetail.tsx

tests/local-boost/
  SecretRedactor.test.ts
  CommandEligibility.test.ts
  CommandTraceWriter.test.ts
  LocalBoostContextStore.test.ts
  LocalBoostEnvBuilder.test.ts
  LocalBoostHookManager.test.ts
  LocalBoostDailyReportGenerator.test.ts
  filters/
...
webpack.config.js                                     — add local-boost-runtime target
src/extension/extension.ts                            — instantiate LocalBoostService
...
src/extension/webview/MessageHandler.ts               — add localBoost message handlers
src/extension/webview/CodexMessageHandler.ts          — add localBoost message handlers (TBD: verify)
src/extension/analytics/ProjectAnalyticsStore.ts      — add LocalBoostSessionStats to session summaries
src/extension/types/webview-messages.ts               — add localBoost message types
src/webview/state/store.ts                            — add localBoost state slice
src/webview/hooks/useClaudeStream.ts                  — forward localBoost messages to store
...
Create `Kingdom_of_Claudes_Beloved_MDs/LOCAL_BOOST.md` containing:
- What Local Boost does and why it exists
...
Add an index entry for Local Boost:
```
**[Local Boost]** — Local-only command output compressor that reduces noisy Bash output before it reaches coding agents. Redacts secrets, filters noise, preserves exit codes, and tracks local-only metrics.
> Detail: `Kingdom_of_Claudes_Beloved_MDs/LOCAL_BOOST.md`
```

Update the directory structure section to include `src/extension/local-boost/`, `src/local-boost-runtime/`, and `src/webview/components/LocalBoost/`.

Update the settings table to include the new `claudeMirror.localBoost.*` settings.

Update the dependencies table if any new packages were added.

### 13.3 Acceptance

- `LOCAL_BOOST.md` exists and documents the current implementation
