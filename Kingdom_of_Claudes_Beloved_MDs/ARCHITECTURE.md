...
MessageHandler tracks tool names during streaming (`currentMessageToolNames`). When `messageDelta` fires with `stopReason === 'tool_use'` and one of the tools is `ExitPlanMode` or `AskUserQuestion`, it sends a `planApprovalRequired` message to the webview with the `toolName`. The approval bar is shown to the user. For `AskUserQuestion`, the user's response is sent as a plain text message to the CLI via stdin (with several questions, readable question+answer pairs built from `questionAnswers`). For `ExitPlanMode`, approve actions do NOT send any user message (see below).
...
**MessageList** - Scrollable container with auto-scroll. Pauses auto-scroll when user scrolls up (>100px from bottom). Shows a floating "scroll to bottom" arrow button when the user has scrolled up; clicking it smooth-scrolls back to the latest message. Renders completed messages and streaming blocks. Subscribes to the store with field-level selectors (not a bare `useAppStore()`), so it only re-renders when chat-relevant slices change — unrelated store updates (tab list broadcasts, widgets) do not touch it.

**MessageBubble** - Renders a completed message. Wrapped in `React.memo`: message objects keep their identity once finalized and the `MessageList` callbacks are `useCallback`-stable, so list-level re-renders skip untouched bubbles (markdown parsing and code highlighting do not re-run). Parses text content to extract fenced code blocks (``` delimiters). Uses `dir="auto"` for automatic bidi direction. Renders tool_use, tool_result, and image content blocks inline. Tool result blocks are rendered with a collapsible panel (collapsed by default) to keep the chat clean - file paths inside tool results are also clickable. Image blocks display as responsive thumbnails with rounded borders. File paths in plain text segments are detected and rendered as clickable links via `renderTextWithFileLinks()`. A **Copy button** appears on hover in the message role header for both user and assistant messages, copying the full text content to the clipboard (uses Clipboard API with `execCommand` fallback). Shows "Copied!" confirmation for 2 seconds. Each message displays an **HH:MM timestamp** next to the role label, formatted from the `ChatMessage.timestamp` field using the user's locale. Streaming messages also show the current time.
...
- **AskUserQuestion**: Displays ALL questions parsed from the tool's JSON input (`planText`), up to 4 per call. A single question keeps the fast UX: single-select sends immediately on click, multi-select shows checkboxes with a Submit button, and a global "Custom answer..." area covers free text. Multiple questions render as stacked question groups with per-question state (radio-style single-select, checkbox multi-select, per-question "Custom answer..." toggle), an `answered/total` progress line, and one Submit button that unlocks only when every question has an answer. Answers are sent per-question via `questionAnswers` on `planApprovalResponse` so no question silently falls back to its first option.
...
- **File paths**: Windows absolute (`C:\...`), Unix absolute (`/...`), and relative paths (`src/...`, `./...`). Paths with `:line` or `:line:col` suffixes are also matched (e.g., `file.ts:42`). The extension-side `openFile` parser also supports GitHub-style anchors (`file.ts#L42`, `file.ts#L42C7`) and trims wrapper/leading punctuation tokens (for examples like `:file.ts#L42`). Relative paths are resolved against workspace root, with fallback basename/suffix search (including parent-folder search when the workspace root is `.xcodeproj`/`.xcworkspace`), then opened at the resolved location and line/column.
