...
MessageHandler (case 'workstreamMapReclassify')
  -> scopes sessions (open tabs + last 4 days of activity)
  -> calls workstreamManager.classifyProject(..., allSummaries)
  -> manager ingests orphan git commits (14 days) via GitCommitIngestor
...
- `source?: 'session' | 'external_folder' | 'git_history'` plus `sourceFolderPath`, `sourceFilePaths`, `sourceImportedAt`, `sourceDocumentCount`, `sourceTotalBytes` for imported external work
...
2. **Recent sessions** -- Sessions from `ProjectAnalyticsStore` with last activity (endedAt, falling back to startedAt) within the last 4 days
3. **Git-inferred sessions** -- Orphan git commits (not associated with any ClaUi session) from the last 14 days are ingested as synthetic sessions by `GitCommitIngestor`
...
| `GitCommitIngestor` (git log) | Current branch, last 14 days | 10 synthetic sessions | Catches work done by external tools (Codex, other CLIs) not tracked by ClaUi sessions |
...
Log output: `Scoped N total summaries -> M (openTabs=X, recentCutoff=4d)`

### Git History Ingestion (WorkstreamManager)
After session enrichment, `GitCommitIngestor` reads `git log --since="14 days ago" --no-merges --name-only` and:
1. Filters out commits whose hash matches any `SessionSummary.gitCommit` across ALL stored sessions (not just the 4-day window)
2. Filters out commits whose timestamp falls within any session's `startedAt`-`endedAt` window
3. Groups remaining orphan commits by temporal proximity (4-hour gap) and file overlap (>20%)
4. Converts each group into a synthetic `EnrichedSessionData` with `source: 'git_history'`
5. Caps at 10 synthetic sessions (most recent first)
6. Merges with enriched ClaUi sessions before classification
...
2. **Git ingest** -- `GitCommitIngestor` discovers orphan commits (last 14 days, not in any session) and creates synthetic `EnrichedSessionData` entries with `source: 'git_history'`. Merged with enriched sessions before classification
3. **Pre-cluster** -- `WorkstreamClassifier.heuristicPreCluster()` groups by:
   - Git branch (any branch including main/master, 2+ sessions)
   - File overlap (>= 0.5 on modified + read files)
   - Temporal proximity (sessions within 2 hours with >30% word overlap in prompts)
4. **Classify** -- Sonnet AI classifies sessions into workstreams via CLI call. The prompt instructs the AI that sessions on the same branch (including main) may belong to different workstreams, and that `git_history` sessions are real work from external tools. Respects existing user edits via protected assignments
5. **Label sanitization** -- Post-classification, workstream labels are stripped of the word "ultrathink" (regex `/\bultrathink\b\s*/gi`), since it is a thinking activation keyword not part of actual work content. The classification prompt also instructs the model to exclude it.
6. **Extract stations** -- `StationExtractor` extracts 1-5 stations per session via Sonnet (batched)
7. **Synthesize** -- `CurrentStateSynthesizer` generates project + per-workstream current state
8. **Score** -- `WorkstreamImportanceScorer` computes importance/attention scores
9. **Save** -- `WorkstreamStore` persists to `workspaceState`, captures snapshot
...
### Layout Algorithm
- Workstreams sorted by: pinned first, then by most recent activity (last change descending), then by status (blocked > active > uncertain > research > planning > completed > abandoned), then by importance score
