...
Right-clicking any clickable link in the chat opens the shared chat context menu (`BtwContextMenu`) with an extra **"Copy link"** item above the usual Copy/Paste/btw items. `MessageList.handleContextMenu()` resolves the clicked link via `extractLinkUrl()`, which matches all link representations — markdown anchors (`a.md-link`, target in `data-href`/`href`), URL spans (`.url-link`, `data-url`), and file-path links (`.file-path-link`, `data-path`) — falling back to the element's text content for the React-rendered variants in code blocks / tool results (`filePathLinks.tsx`), which carry no dataset. The item appears for both web URLs and file paths; it is hidden only when the right-clicked target is not a link. Selecting it copies the target string by posting `copyToClipboard` to the extension host, which writes it via `vscode.env.clipboard` (handled in both `MessageHandler` and `CodexMessageHandler`).
