...
- `src/extension/session/TabManager.ts`
  ...
  - `applyTabLayout(mode)` routes horizontal vs vertical behavior and calls `syncNativeTabVisibility()`. **Fast path**: when the window already has a single editor group (the common case for every tab created after the first), it skips `joinAllGroups`, the empty-group sweep, the settle delays, and the re-reveal — only the (guarded) native-tab sync and one debounced broadcast run.
  - `syncNativeTabVisibility(mode)` hides VS Code's native editor tabs (`workbench.editor.showTabs` -> `'none'`, **workspace scope — this window only**) when entering vertical mode and restores the previous workspace-level value (usually unset) when switching back to horizontal. The write is skipped when the workspace value is already `'none'` (it runs on every vertical-mode tab creation; a settings write hits disk and fans a config-change event out to every extension). Saved/restored on extension shutdown; a leftover workspace `'none'` found while capturing is treated as ours (crash remnant) and restored to unset.
  - `showNativeTabsWhileDocumentActive()` temporarily restores the native strip while a document tab is active in vertical mode (the rail lives inside the covered webview); `handleTabFocused()` re-hides it when a ClaUi panel regains focus.
  ...
  - `broadcastTabsState()` sends the open tab list, active tab id, and the persisted vertical-rail width to every webview as `tabList`. **Debounced (50ms trailing)**: a single tab open/close/focus fires many `onDidChangeTabs`/`onDidChangeTabGroups` events back-to-back, and each broadcast makes every live webview process a full `tabList` message — bursts coalesce into one broadcast. On the webview side, `setOpenTabs` (store) deep-compares the incoming payload against current state and skips the store update entirely when identical, so redundant broadcasts re-render nothing.
  - `getVerticalTabRailWidth()` / `setVerticalTabRailWidth(width)` read/write the shared rail width in **globalState** (key `claudeMirror.verticalTabRailWidth`, clamped 80-300, `null` = CSS default). The setter is a no-op when the value is unchanged; otherwise it persists and re-broadcasts so every tab's rail adopts the width immediately. Because the width lives in globalState (not the disposable webview store), it survives reloads, tab switches, and horizontal<->vertical toggles.
  - `restoreFromSnapshot()` recreates tabs in one column first, then applies the selected layout after restore.
- `src/extension/commands/tabGroupCommands.ts` - `claudeMirror.tabs.openLayoutMenu` QuickPick plus `claudeMirror.tabs.refreshList`, `claudeMirror.tabs.setRailWidth` (both internal commands, not in package.json contributes), `claudeMirror.tabs.close`, and `claudeMirror.tabs.reorder` commands. `setRailWidth` forwards the width to `TabManager.setVerticalTabRailWidth()`.
- `src/extension/webview/MessageHandler.ts` + `CodexMessageHandler.ts` + `MultiParticipantSessionTab.ts`
  ...
  - `setVerticalTabRailWidth` message routes to the `claudeMirror.tabs.setRailWidth` command (persists the shared rail width in globalState + re-broadcasts).
...
- `src/webview/App.tsx`
  - `VerticalTabRail` renders the left-side in-webview tab navigator when vertical mode is active and there is more than one tab OR any folder exists. Includes a draggable resize handle on the right edge (80px-300px, double-click to reset). Dragging updates the local width live; on mouseup (or double-click reset) it posts `setVerticalTabRailWidth` to the extension, which persists the width globally and re-broadcasts it to every tab. A `verticalTabRailResizing` store flag is set for the duration of the drag so an incoming `tabList` broadcast cannot clobber the in-progress width. Tabs render grouped under collapsible folder headers (built by `src/webview/tabNav.ts`); ungrouped tabs list first without a header. Tab drag-and-drop posts `moveTabInNavigation` (target folder + index); header click posts `setGroupCollapsed`; the "+ Folder" button posts `createTabGroup`. The provider letter (C/X/H) becomes a red close button on hover.
...
- `src/webview/state/store.ts` - `tabLayout`, `verticalTabRailWidth`, `verticalTabRailResizing`, `openTabs`, `activeTabId`, `tabGroups`, `collapsedGroupIds`, `setTabLayout()`, `setVerticalTabRailWidth()`, `setVerticalTabRailResizing()`, and `setOpenTabs()` (carries tabs + groups + collapse state + open documents; no-ops when the incoming payload deep-equals current state so redundant `tabList` broadcasts trigger no re-render). `verticalTabRailWidth` is a mirror of the extension-owned value; the source of truth is globalState.
- `src/webview/hooks/useClaudeStream.ts` - handles inbound `tabLayoutSetting` and `tabList` (tabs + groups + collapsedGroupIds + `verticalTabRailWidth`, the latter skipped while `verticalTabRailResizing` is true); sends `requestTabList` on webview ready.
...
- **Files open as regular tabs in the panels' editor group — no side split.** `handleOpenFile` (MessageHandler / CodexMessageHandler) opens files into the active group in both layouts, so in vertical mode a file joins the same group as the ClaUi panels and its content fills the panel area, exactly like switching tabs. Because the rail renders inside the ClaUi webview, an active document covers it — so `TabManager.showNativeTabsWhileDocumentActive()` (wired to `onDidChangeTabs`/`onDidChangeTabGroups`) temporarily restores the native tab strip while a document tab is active, and `handleTabFocused()` re-hides it the moment a ClaUi panel becomes active again. Navigation loop: rail file row / chat link -> file shows with the native strip on top -> click the ClaUi tab in the strip (or close the file) -> rail returns and the strip hides.
- **Open documents are first-class rail rows.** The rail lists open documents as regular tab rows (same style as session tabs, gray accent border, "F" badge that turns into the red close X on hover), rendered right after the ungrouped session tabs. `TabManager.listOpenDocuments()` enumerates `vscode.window.tabGroups` (text, diff, notebook, custom inputs; ClaUi webviews excluded) into the `tabList` broadcast as `openDocuments`; `onDidChangeTabs`/`onDidChangeTabGroups` re-broadcast on every open/close/dirty change. Click posts `focusDocument` (routes to `claudeMirror.docs.focus` -> `vscode.open` in the document's own column); the X posts `closeDocument` (routes to `claudeMirror.docs.close` -> `vscode.window.tabGroups.close`). Dirty documents show a `*` prefix. Open documents also count toward showing the rail (so a single session tab + one file still gets a rail). Documents are not draggable into folders. Keyboard `Ctrl+W` still closes the active editor as usual.
- The vertical rail width is resizable by dragging the handle on its right edge. Double-click resets to the CSS default (`clamp(96px, 28vw, 132px)`). The chosen width is **persisted in globalState** by the extension and **shared across all tabs** (every rail uses the same width). It is applied as a CSS variable override from the mirrored `verticalTabRailWidth` store field. Because the source of truth is globalState — not the per-webview Zustand store — the width survives switching between tabs, webview reloads, and horizontal<->vertical toggles (previously the width was store-only and reset to the default on any of these).
