...
- `src/webview/state/store.ts` - `tabLayout`, `verticalTabRailWidth`, `openTabs`, `activeTabId`, `tabGroups`, `collapsedGroupIds`, `setTabLayout()`, `setVerticalTabRailWidth()`, and `setOpenTabs()` (carries tabs + groups + collapse state + open documents; no-ops when the incoming payload deep-equals current state so redundant `tabList` broadcasts trigger no re-render).
