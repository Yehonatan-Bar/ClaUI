...
1. ההבדל בין הפרומפט שהמשתמש מקליד לבין הפלט של Claude.
2. ההבדל בין שלבי הביניים בתהליך (שימוש בכלים) לבין התשובה הסופית.

ה־Theme שומר על פלטת הצבעים של VS Code שבה המשתמש משתמש, ומוסיף עליה הדגשות סמנטיות חזקות.

### קידוד צבעים

- **כחול** = הקלט שלך (הודעת משתמש)
- **סגול** = הפלט של Claude (עבודה/נרטיב ביניים)
- **ירוק** = התשובה הסופית

### אבחנה בין משתמש ל-Claude

- כל הודעת משתמש מקבלת כרטיס כחול עם פס שמאלי חזק ותווית `YOU` כצ'יפ צבעוני.
- כל הודעת Claude מקבלת משטח סגול נפרד עם תווית `ASSISTANT` כצ'יפ.
