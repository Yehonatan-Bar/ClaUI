...
import {
  LocalBoostTrace, LocalBoostContextFile, FilterConfig,
  CLAUI_LOCAL_BOOST_SCHEMA_VERSION,
} from '../extension/local-boost/LocalBoostTypes';
...
async function main(): Promise<void> {
  ...
  if (encodedIdx !== -1 && args[encodedIdx + 1]) {
    ...
    process.stderr.write('[claui-local-boost] Usage: claui-run --claui-encoded-shell-command <base64url> | claui-run -- <command...>\n');
    process.exit(1);
  }

  if (!command) {
    process.stderr.write('[claui-local-boost] Runner failed before executing command: empty command\n');
    process.exit(127);
  }

  // Read env vars
  const contextFilePath = process.env.CLAUI_LOCAL_BOOST_CONTEXT_FILE ?? '';
  const storeDir = process.env.CLAUI_LOCAL_BOOST_STORE_DIR ?? '';
  const shellOverride = process.env.CLAUI_LOCAL_BOOST_SHELL;
  const filterProfile = (process.env.CLAUI_LOCAL_BOOST_FILTER_PROFILE ?? 'balanced') as 'balanced' | 'strict' | 'verbose';
  const storeRawLogs = process.env.CLAUI_LOCAL_BOOST_STORE_RAW_LOGS !== 'false';

  // Read context file
  let context: LocalBoostContextFile | null = null;
  if (contextFilePath) {
    try {
  ...
  try {
    ...
    redactedStdout = '[claui-local-boost] Output suppressed: redaction error.';
    redactedStderr = '';
  ...
  try {
    ...
    filterOutput = {
      ...
      header: `[claui-local-boost] ${command} exited ${result.exitCode} in ${(result.durationMs / 1000).toFixed(1)}s. Filter error: fallback used.`,
  ...
  if (storeDir) {
    ...
    const trace: LocalBoostTrace = {
      schemaVersion: CLAUI_LOCAL_BOOST_SCHEMA_VERSION,
      traceId,
    ...
    try {
      ...
      process.stderr.write('[claui-local-boost] Warning: failed to write trace file\n');
    }
  }
...
main().catch((err) => {
  process.stderr.write(`[claui-local-boost] Runner failed before executing command: ${err?.message ?? err}\n`);
  process.exit(127);
});
