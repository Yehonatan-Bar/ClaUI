...
import { LocalBoostContextFile, CLAUI_LOCAL_BOOST_SCHEMA_VERSION } from './LocalBoostTypes';
...
export class LocalBoostContextStore {
  ...
  async createContext(
    ...
    const context: LocalBoostContextFile = {
  ...
  private async updateField(tabRuntimeId: string, fields: Partial<LocalBoostContextFile>): Promise<void> {
    const contextPath = this.getContextPath(tabRuntimeId);
    try {
