...
import {
  LocalBoostTrace, LocalBoostTraceSummary, LocalBoostAggregate,
  LocalBoostDailyReport, LocalBoostSettings,
} from './LocalBoostTypes';

export class LocalBoostTraceReader {
  constructor(private storeDir: string) {}

  async getRecentTraces(limit: number, workspacePath?: string): Promise<LocalBoostTraceSummary[]> {
    const tracesDir = path.join(this.storeDir, 'traces');
    const dateDirs = await this.getSortedDateDirs(tracesDir);
    const summaries: LocalBoostTraceSummary[] = [];
    ...
    for (const dateDir of dateDirs) {
      ...
      for (const file of jsonFiles) {
        ...
        try {
          ...
          const trace: LocalBoostTrace = JSON.parse(raw);
  ...
  async getTrace(traceId: string): Promise<LocalBoostTrace | null> {
    const tracesDir = path.join(this.storeDir, 'traces');
    const dateDirs = await this.getSortedDateDirs(tracesDir);
  ...
  async getAggregate(workspacePath?: string): Promise<LocalBoostAggregate> {
  ...
  async getDailyReport(date: string): Promise<LocalBoostDailyReport | null> {
  ...
  async cleanExpired(settings: LocalBoostSettings): Promise<{
  ...
  private toSummary(trace: LocalBoostTrace): LocalBoostTraceSummary {
    return {
  ...
  private computeAggregate(summaries: LocalBoostTraceSummary[]): LocalBoostAggregate {
