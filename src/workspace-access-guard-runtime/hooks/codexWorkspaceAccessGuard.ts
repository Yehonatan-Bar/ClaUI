...
function main(): void {
  ...
  const timer = setTimeout(() => {
    if (eventName === 'PreToolUse' || eventName === 'PermissionRequest') {
      deny('Workspace Access Guard timeout exceeded', eventName);
    } else { allow(); }
    process.exit(0);
  }, TIMEOUT_MS);
  ...
  process.stdin.on('end', () => {
    ...
    try {
      ...
      if ((eventName === 'PreToolUse' || eventName === 'PermissionRequest') && settings.mode === 'block') {
        deny('Failed to parse hook input', eventName);
      } else {
        allow();
      }
    }
...
function handleEvent(eventName: string, input: HookInput, settings: WorkspaceAccessGuardSettings): void {
  if (eventName !== 'PreToolUse' && eventName !== 'PermissionRequest') { allow(); return; }
  ...
  if (toolName === 'Bash') {
    ...
    const patchText = (toolInput.patch as string) ?? (toolInput.diff as string) ?? '';
    extractedPaths = extractPatchFilePaths(patchText);
    operation = 'write';
  } else if (toolName.startsWith('mcp__')) {
    if (!settings.scanMcpTools) { allow(); return; }
    const result = extractToolPaths({ provider: 'codex', toolName, toolInput, cwd });
    extractedPaths = result.paths;
    operation = 'mcp';
  } else {
    if (!settings.scanFileTools) { allow(); return; }
  ...
  if (decision.action === 'deny') {
    deny(decision.remediation ?? decision.reason, eventName);
  } else {
    allow();
  }
}
...
function extractPatchFilePaths(patch: string): string[] {
  ...
  for (const line of lines) {
    const diffMatch = /^(?:---|\+\+\+)\s+(?:a\/|b\/)?(.+)$/.exec(line);
    if (diffMatch && diffMatch[1] !== '/dev/null') {
      const fp = diffMatch[1].trim();
      if (fp && !paths.includes(fp)) paths.push(fp);
    }
  }
  return paths;
}
...
function deny(reason: string, eventName = 'PreToolUse'): void {
  const output = {
    hookSpecificOutput: {
