...
function parseHookEvent(): SpaHookEvent {
  ...
  if (!['PreToolUse', 'PostToolUse', 'Stop', 'PermissionRequest'].includes(event)) {
...
async function handlePreToolUse(
  ...
  if (/^(Edit|Write|MultiEdit|apply_patch)$/.test(toolName) && settings.scanEditTools) {
    const filePath = (input.tool_input.file_path || input.tool_input.path) as string | undefined;
    const rawContent = (input.tool_input.content || input.tool_input.new_string || input.tool_input.patch || '') as string;
    ...
    const findings = scanner.scan({
      text: content, source: 'edit', provider: 'codex',
      filePath, cwd: process.cwd(), sessionId: process.env.CLAUI_SESSION_ID,
    });

    let isFileGitIgnored: boolean | undefined;
    if (filePath && settings.allowIgnoredEnvFiles) {
      const gitScanner = new GitStateScanner(scanner, process.cwd(), 'codex');
      isFileGitIgnored = gitScanner.isGitIgnored(filePath);
    }

    const exceptions = exceptionLoader.loadActive();
    const decision = policy.evaluate({
      findings, filePath, source: 'edit', provider: 'codex',
      toolName, cwd: process.cwd(), settings, exceptions, isFileGitIgnored,
    });

    if (decision.consumedExceptionIds.length > 0) exceptionLoader.consumeMany(decision.consumedExceptionIds);
    if (decision.action !== 'allow') {
      audit.write(buildAuditEvent(decision, toolName, 'edit', 'codex', process.env.CLAUI_SESSION_ID, undefined, filePath));
    }
    if (decision.action === 'deny') {
      return deny(`Super Particle Accelerator blocked this action.\n\n${decision.reason}\n\n${decision.remediation ?? ''}`);
    }
    return allow();
  }
  ...
  if (toolName === 'Bash' && settings.scanBashCommands) {
    ...
    if (settings.blockGitCommitPush && isGitDeployCommand(command)) {
      ...
      if (allFindings.length > 0) {
        const exceptions = exceptionLoader.loadActive();
        const decision = policy.evaluate({
          findings: allFindings, source: 'staged-diff', provider: 'codex',
          toolName: 'Bash', command, cwd: process.cwd(), settings, exceptions,
        });
        if (decision.action === 'deny') {
          audit.write(buildAuditEvent(decision, 'Bash', 'staged-diff', 'codex', process.env.CLAUI_SESSION_ID));
          return deny(`Super Particle Accelerator blocked this git operation.\n\n${decision.reason}\n\n${decision.remediation ?? ''}`);
        }
      }
    }
    ...
    if (isBashWriteCommand(command)) {
      const findings = scanner.scan({
        text: command, source: 'bash-command', provider: 'codex',
        cwd: process.cwd(), sessionId: process.env.CLAUI_SESSION_ID,
      });
      if (findings.length > 0) {
        const exceptions = exceptionLoader.loadActive();
        const decision = policy.evaluate({
          findings, source: 'bash-command', provider: 'codex',
          toolName: 'Bash', command, cwd: process.cwd(), settings, exceptions,
        });
        if (decision.action === 'deny') {
          audit.write(buildAuditEvent(decision, 'Bash', 'bash-command', 'codex', process.env.CLAUI_SESSION_ID));
          return deny(`Super Particle Accelerator blocked this command.\n\n${decision.reason}\n\n${decision.remediation ?? ''}`);
        }
      }
    }
  ...
  if (toolName.startsWith('mcp__') && settings.scanMcpTools) {
    ...
    const findings = scanner.scan({
      text, source: 'mcp-args', provider: 'codex',
      cwd: process.cwd(), sessionId: process.env.CLAUI_SESSION_ID,
    });
    ...
    const exceptions = exceptionLoader.loadActive();
    const decision = policy.evaluate({
      findings, source: 'mcp-args', provider: 'codex',
      toolName, cwd: process.cwd(), settings, exceptions,
    });
    if (decision.action !== 'allow') {
      audit.write(buildAuditEvent(decision, toolName, 'mcp-args', 'codex', process.env.CLAUI_SESSION_ID));
    }
    if (decision.action === 'deny') {
      return deny(`Super Particle Accelerator blocked this MCP call.\n\n${decision.reason}\n\n${decision.remediation ?? ''}`);
    }
    return allow();
  }
...
async function handleStop(
  ...
  const gitScanner = new GitStateScanner(scanner, cwd, 'codex');
  ...
  const exceptions = exceptionLoader.loadActive();
  const decision = policy.evaluate({
    findings: newFindings, source: 'diff', provider: 'codex',
    cwd, settings, exceptions,
  });
  ...
  baselineStore.save(sessionId, allFindings);
  audit.write(buildAuditEvent(decision, 'Stop', 'diff', 'codex', sessionId));
