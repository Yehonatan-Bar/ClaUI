...
function toneForStatus(status: string, lastEvent?: { action: string }): StatusBadgeTone {
  ...
  if (status === 'disabled-drift' || status === 'error' || status === 'enabled-hooks-missing') return 'error';
...
export const SuperParticleAcceleratorStatusBadge: React.FC = () => {
  ...
  const label = 'SPA';
  const value = !enabled
    ? (status === 'disabled-drift' ? 'Stale' : 'Off')
    : status === 'enabled-hooks-installed'
      ? (mode === 'audit' ? 'Audit' : 'On')
      : 'Warn';

  const titleMap: Record<string, string> = {
    disabled: 'Super Particle Accelerator: Disabled',
    'disabled-drift': 'Super Particle Accelerator: Disabled in settings, but stale hooks remain on disk - open the panel to force cleanup',
    'enabled-hooks-installed': mode === 'audit'
      ? 'Super Particle Accelerator: Active (audit - logs only, never blocks)'
      : 'Super Particle Accelerator: Active (block mode)',
