# ClaUi - Development Guide

This document covers building, developing, and contributing to ClaUi. For user-facing documentation, see [README.md](README.md).
...
```bash
git clone https://github.com/Yehonatan-Bar/ClaUI.git
cd ClaUI
npm install
```
...
Each tab creates its own CLI process (`claude -p --output-format stream-json`), stream parser, and React webview panel. A `TabManager` coordinates multiple independent sessions running in parallel.
...
code --install-extension claude-code-mirror-<version>.vsix --force
```
...
**Known triggers**: VS Code reload, VS Code updates, certain window layouts.
