...
- רב־תורי חי עם זיכרון שיחה על שני השערים (Ox Alpha ב-OpenRouter, Big Pickle ב-OpenCode Zen).

## לקחים מהשטח — חשוב לכל מי שמארח את זה (כולל אימוץ נייטיבי)

שלושה באגים אמיתיים שצצו ביום הראשון של שימוש חי, כולם מתוקנים כאן:

1. **tools + היסטוריה = ריק ב-ox-alpha** — ה-upstream מחזיר HTTP 200 עם content ריק
   (`native_finish_reason: "network_error"`) כשמצרפים `tools` לבקשה רב־תורית. התור הראשון
   עובד, כל השאר ריקים. הטיפול: תשובה ריקה בזמן ש-tools מצורפים → retry מיידי בלי tools
   והסשן נשאר צ'אט־בלבד. אל תניחו ש-200 = הצלחה בשערים חינמיים.
2. **round-trip של מזהה המודל** — המארח (ClaUi) שומר את מזהה התצוגה מהודעת ה-init ומחזיר
   אותו כ---model בהפעלה הבאה. אם מזהה התצוגה שונה מהקנוני — ה-resolver חייב לקבל את שניהם,
   אחרת טאב שחוזר מ-resume נשבר עם "not a valid model ID".
3. **היסטוריה לא־מתחלפת** — כשמודל מחזיר ריק, נשארות הודעות user רצופות בהיסטוריה, וחלק
   מהשערים משתבשים מזה. לפני שליחה ממזגים הודעות user עוקבות לאחת.
